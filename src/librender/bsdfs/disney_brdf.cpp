...
public:
    ...
    Color3 eval(const BSDFContext &ctx, const SceneInteraction &si,
        ...
        auto c_spec =
            lerp(metallic, specular * .8f * lerp(specular_tint, 1.f, c_tint),
                 c_dlin);
        // Computes
        ...
        Float fl    = schlick_weight(cos_theta_i),
              fv    = schlick_weight(cos_theta_o);
        ...
        Float f_subsurface =
            1.25 * (f_ss * (1 / (cos_theta_i + cos_theta_o) - .5) + .5);
        // Specular
        Float alpha = math::safe_sqrt(1.f - 0.9f * anisotropic);
        Float a_x   = std::max(.001f, math::sqr(roughness) / alpha),
              a_y   = std::max(.001f, math::sqr(roughness) * alpha);
        Float Ds  = microfacet::gtr2_aniso(sin_theta_h, cos_theta_h, sin_phi_h,
                                          cos_phi_h, a_x, a_y);
        Color3 Fs = lerp(c_spec, 1.f, schlick_weight(cos_theta_d));
        Float Gs  = microfacet::smith_g1_ggx_aniso(tan_theta_i, sin_phi_i,
                                                  cos_phi_i, a_x, a_y) *
                   microfacet::smith_g1_ggx_aniso(tan_theta_o, sin_phi_o,
                                                  cos_phi_o, a_x, a_y);
        Color3 f_specular = Fs * Ds * Gs;
        // Sheen
        Color3 f_sheen =
            lerp(1.f, c_tint, sheen_tint) * schlick_weight(cos_theta_d) * sheen;
        // Clearcoat
        Float Dc = microfacet::gtr1(cos_theta_h,
                                    math::lerp(.1f, .001f, clearcoat_gloss));
        Float Fc = math::lerp(.04f, 1.f, schlick_weight(cos_theta_d));
        Float Gc = microfacet::smith_g1_ggx(cos_theta_i, 0.25) *
                   microfacet::smith_g1_ggx(cos_theta_o, 0.25);
        Color3 f_clearcoat = .25 * clearcoat * Gc * Fc * Dc;
        // Result
        Color3 f_diffuse =
            ((1 / math::Pi<Float>) *math::lerp(f_d, f_subsurface, subsurface) *
                 c_dlin +
             f_sheen) *
            (1 - metallic);
    ...
    Float pdf(const BSDFContext &ctx, const SceneInteraction &si,
        ...
        auto pdf_specular = microfacet::smith_g1_ggx_aniso(
                                tan_theta_i, sin_phi_i, cos_phi_i, a_x, a_y) *
                            specular_d / (4 * cos_theta_i);
        const Float clearcoat_d =
            microfacet::gtr1(cos_theta_h, clearcoat_gloss);
        auto pdf_clearcoat = cos_theta_h * clearcoat_d / (4 * cos_theta_d);
        if (has_diffuse && has_specular) {
            pdf = prob_diffuse * pdf_diffuse +
                  (1.f - prob_diffuse) * (prob_specular * pdf_specular +
                                          prob_clearcoat * pdf_clearcoat);
        } else if (has_diffuse) {
            pdf = pdf_diffuse;
        } else {
...
private:
    ...
    Vector3 sample_specular(const Vector3 &wi, const Vector2 &sample, Float ax,
                            Float ay) const {
    ...
    Vector3 sample_clearcoat(const Vector3 &wi, const Vector2 &sample,
    ...
    Color3 mon2lin(const Color3 &x) const {
        return Color3(std::pow(x[0], 2.2), std::pow(x[1], 2.2),
                      std::pow(x[2], 2.2));
    }

    std::shared_ptr<Texture> m_base_color, m_subsurface, m_metallic, m_specular,
        m_specular_tint;
