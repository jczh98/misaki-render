...
public:
    ...
    std::pair<BSDFSample, Color3> sample(const BSDFContext &ctx,
        ...
        BSDFSample bs;
        if (cos_theta_i <= 0.f || !ctx.is_enabled(BSDFFlags::DeltaReflection))
            return { bs, 0.f };
        bs.wo                = reflect(si.wi);
        bs.pdf               = 1.f;
        bs.eta               = 1.f;
        ...
        Color3 value         = m_specular_reflectance->eval_3(si.geom) *
                       fresnel_conductor(cos_theta_i, m_eta->eval_3(si.geom),
                                         m_k->eval_3(si.geom));
        return { bs, value };
    }
