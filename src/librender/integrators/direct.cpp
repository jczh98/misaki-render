...
public:
    ...
    bool render(const std::shared_ptr<Scene> &scene) {
        ...
        tbb::parallel_for(
            ...
            [&](const tbb::blocked_range<size_t> &range) {
                ...
                auto block =
                    std::make_unique<ImageBlock>(m_block_size, film->filter());
                    ...
                    for (int y = 0; y < size.y(); ++y) {
                        for (int x = 0; x < size.x(); ++x) {
                            ...
                            for (int s = 0; s < total_spp; ++s) {
                                ...
                                auto [ray, ray_weight] =
                                    camera->sample_ray(position_sample);
                                auto result = sample(scene, sampler.get(), ray);
                                if (result)
                                    block->put(position_sample,
                                               Color4(*result));
                                else
                                    block->put(position_sample,
    ...
    std::optional<Color3> sample(const std::shared_ptr<Scene> &scene,
                                 Sampler *sampler, const Ray &ray_) const {
        auto ray = ray_;
        Color3 result(0.f);
        ...
        if (has_flag(bsdf->flags(), BSDFFlags::Diffuse)) {
            for (size_t i = 0; i < m_light_samples; ++i) {
                auto [ds, emit_val] =
                    scene->sample_direct_light(si.geom, sampler->next2d());
                if (ds.pdf != 0.f) {
                    auto wo         = si.to_local(ds.d);
                    Color3 bsdf_val = bsdf->eval(ctx, si, wo);
                    auto bsdf_pdf   = bsdf->pdf(ctx, si, wo);
                    Float mis       = ds.geom.degenerated
                                          ? 1.f
                                          : mis_weight(ds.pdf * m_frac_lum,
                                                 bsdf_pdf * m_frac_bsdf) *
                                          m_weight_lum;
        ...
        for (size_t i = 0; i < m_bsdf_samples; ++i) {
            ...
            if (bsdf_val != 0.f) {
                auto si_bsdf =
                    scene->ray_intersect(si.spawn_ray(si.to_world(bs.wo)));
                ...
                if (light != nullptr) {
                    ...
                    auto light_pdf =
                        !has_flag(bs.sampled_type, BSDFFlags::Delta)
                            ? scene->pdf_direct_light(si.geom, ds, light)
                            : 0.f;
                    Float mis = mis_weight(bs.pdf * m_frac_bsdf,
                                           light_pdf * m_frac_lum) *
                                m_weight_bsdf;
