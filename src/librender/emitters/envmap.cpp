...
public:
    EnvironmentMap(const Properties &props) : Light(props) {
        ...
        for (int i = 0; i < m_bitmap.data_size(); ++i) {
            luminace[i] = m_bitmap.raw_data()[i].luminance() *
                          std::sin(math::Pi<Float> *
                                   (Float(i) / m_resolution.y() + 0.5f) /
                                   m_resolution.x());
        }
    ...
    void set_scene(const Scene *scene) override {
        ...
        m_bsphere.radius = std::max(
            RayEpsilon<Float>, m_bsphere.radius * (1.f + RayEpsilon<Float>) );
    }
    ...
    sample_direct(const PointGeometry &geom_ref,
        ...
        Float inv_sin_theta = math::safe_rsqrt(std::max(
            math::sqr(d.x()) + math::sqr(d.z()), math::sqr(Epsilon<Float>)));
        d                   = m_world_transform.transform_affine_point(d);
        DirectSample ds;
        ds.geom = PointGeometry::make_infinite(geom_ref.p + d * dist, -d, uv);
        ds.pdf  = pdf > 0.f ? pdf * inv_sin_theta *
                                 (1.f / (2.f * math::sqr(math::Pi<Float>)))
                            : 0.f;
        ds.d    = d;
        ds.dist = dist;
    ...
    Float pdf_direct(const PointGeometry &geom_ref,
        ...
        Float inv_sin_theta = math::safe_rsqrt(std::max(
            math::sqr(d.x()) + math::sqr(d.z()), math::sqr(Epsilon<Float>)));
        return m_dist.pdf(uv) * inv_sin_theta *
               (1.f / (2.f * math::sqr(math::Pi<Float>)));
    }
