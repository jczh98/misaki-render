...
public:
    PointLight(const Properties &props) : Light(props) {
        if (props.has_property("position")) {
            ...
            m_world_transform =
                Transform4f::translate(props.vector3("position"));
        }
    ...
    sample_direct(const PointGeometry &geom_ref,
                  const Vector2 &position_sample) const override {
        DirectSample ds;
        ds.geom =
            PointGeometry::make_degenerated(m_world_transform.translation());
        ds.pdf         = 1.f;
    ...
    Color3 eval(const SceneInteraction &si) const override { return 0.f; }
