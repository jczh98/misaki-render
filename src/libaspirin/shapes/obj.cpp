  ...
  OBJMesh(const Properties &props) : Mesh(props) {
    ...
    while (std::getline(is, line_str)) {
      ...
      if (prefix == "v") {
        ...
        Vector2 tc;
        line >> tc.x() >> tc.y();
        if (filp_tex_coords) tc.y() = 1.f - tc.y();
        texcoords.emplace_back(tc);
      } else if (prefix == "vn") {
        Vector3 n;
        line >> n.x() >> n.y() >> n.z();
        n = math::normalize(m_to_world.apply_normal(n));
        normals.push_back(n);
      } else if (prefix == "f") {
        ...
        for (int i = 0; i < n_vertices; ++i) {
          ...
          std::unordered_map<OBJVertex, uint32_t, OBJVertexHash>::const_iterator it = vertex_map.find(v);
    ...
    m_vertex_size = 3 + 3 + 2;
    m_face_size = 3;
    m_vertices = std::unique_ptr<Float[]>(new Float[(m_vertex_count + 1) * m_vertex_size]);
    m_faces = std::unique_ptr<uint32_t[]>(new uint32_t[(m_face_count + 1) * m_face_size]);
    memcpy(m_faces.get(), triangles.data(), sizeof(uint32_t) * m_face_count * m_face_size);
    for (int i = 0; i < obj_vertices.size(); i++) {
      m_vertices[i * m_vertex_size + 0] = vertices[obj_vertices[i].p - 1].x();
      m_vertices[i * m_vertex_size + 1] = vertices[obj_vertices[i].p - 1].y();
      m_vertices[i * m_vertex_size + 2] = vertices[obj_vertices[i].p - 1].z();
      if (obj_vertices[i].n != -1) {
        m_vertices[i * m_vertex_size + 3] = normals[obj_vertices[i].n - 1].x();
        m_vertices[i * m_vertex_size + 4] = normals[obj_vertices[i].n - 1].y();
        m_vertices[i * m_vertex_size + 5] = normals[obj_vertices[i].n - 1].z();
      ...
      if (obj_vertices[i].uv != -1) {
        m_vertices[i * m_vertex_size + 6] = texcoords[obj_vertices[i].uv - 1].x();
        m_vertices[i * m_vertex_size + 7] = texcoords[obj_vertices[i].uv - 1].y();
