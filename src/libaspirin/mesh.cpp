...
Mesh<Spectrum>::Mesh(const Properties &props) : Shape(props) {
...
template <typename Spectrum>
BoundingBox3 Mesh<Spectrum>::bbox() const {
  return m_bbox;
}

template <typename Spectrum>
BoundingBox3 Mesh<Spectrum>::bbox(uint32_t index) const {
...
template <typename Spectrum>
Float Mesh<Spectrum>::surface_area() const {
  return m_surface_area;
}

template <typename Spectrum>
void Mesh<Spectrum>::area_distr_build() {
...
Mesh<Spectrum>::compute_surface_point(int prim_index, const Vector2 &bary) const {
...
template <typename Spectrum>
std::pair<PointGeometry, Float> Mesh<Spectrum>::sample_position(const Vector2 &sample_) const {
...
template <typename Spectrum>
Float Mesh<Spectrum>::pdf_position(const PointGeometry &geom) const {
