...
Scene<Spectrum>::Scene(const Properties &props) {
...
template <typename Spectrum>
Scene<Spectrum>::~Scene() {
  accel_release();
}

template <typename Spectrum>
std::pair<DirectSample, Color3> Scene<Spectrum>::sample_direct_light(const PointGeometry &geom,
...
template <typename Spectrum>
Float Scene<Spectrum>::pdf_direct_light(const PointGeometry &geom_ref, const DirectSample &ds, const Light *light) const {
  if (m_lights.size() == 1) {
