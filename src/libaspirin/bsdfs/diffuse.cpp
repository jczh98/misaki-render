...
public:
    ...
    sample(const BSDFContext &ctx, const SurfaceInteraction &si, Float sample1,
        ...
        return { bs, bs.pdf > 0.f ? m_reflectance->eval_3(si) : Spectrum::Zero() };
    }
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction &si,
        ...
        if (cos_theta_i > 0.f && cos_theta_o > 0.f) {
            return m_reflectance->eval_3(si) * math::InvPi<Float> *
                   cos_theta_o;
        } else {
            return Spectrum::Zero();
        }
    ...
    std::string to_string() const override {
        ...
        oss << "SmoothDiffuse[" << std::endl
            << "  reflectance = " << string::indent(m_reflectance->to_string()) << std::endl
            << "]";
        return oss.str();
    }
