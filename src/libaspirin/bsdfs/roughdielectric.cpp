...
public:
    ...
    std::pair<BSDFSample, Color3> sample(const BSDFContext &ctx,
        ...
        auto [F, cos_theta_t, eta_it, eta_ti] =
            fresnel(math::dot(si.wi, m), m_eta);
        bool selected_r;
        Color3 weight;
        if (has_reflection && has_transmission) {
        ...
        if (selected_r) {
            ...
            weight *= factor;
            dwh_dwo =
                math::sqr(bs.eta) * math::dot(bs.wo, m) /
                math::sqr(math::dot(si.wi, m) + bs.eta * math::dot(bs.wo, m));
        }
        if (m_sample_visible)
            weight *= distr.smith_g1(bs.wo, m);
        else
            weight *= distr.G(si.wi, bs.wo, m) * math::dot(si.wi, m) /
    ...
    Color3 eval(const BSDFContext &ctx, const SceneInteraction &si,
        ...
        Color3 result = 0.f;
        if (eval_r) {
            result = F * D * G * m_specular_reflectance->eval_3(si.geom) /
        ...
        if (eval_t) {
            Float scale = (ctx.mode == TransportMode::Radiance)
                              ...
                              : 1.f;
            result =
                m_specular_transmittance->eval_3(si.geom) *
                std::abs((scale * (1.f - F) * D * G * eta * eta *
                          math::dot(si.wi, m) * math::dot(wo, m)) /
                         (cos_theta_i * math::sqr(math::dot(si.wi, m) +
                                                  eta * math::dot(wo, m))));
        }
        return result;
    }
    ...
    Float pdf(const BSDFContext &ctx, const SceneInteraction &si,
        ...
        Vector3 m    = math::normalize(si.wi + wo * (reflect ? 1.f : eta));
        m *= std::copysign(1.f, Frame::cos_theta(m));
        if (math::dot(si.wi, m) * Frame::cos_theta(si.wi) <= 0.f ||
            math::dot(wo, m) * Frame::cos_theta(wo) <= 0.f)
            return 0.f;
        Float dwh_dwo =
            (reflect
                 ? 1.f / (4.f * math::dot(wo, m))
                 : (eta * eta * math::dot(wo, m)) /
                       math::sqr(math::dot(si.wi, m) + eta * math::dot(wo, m)));
        MicrofacetDistribution sample_distr(m_type, m_alpha_u->eval_1(si.geom),
                                            m_alpha_v->eval_1(si.geom),
                                            m_sample_visible);
        if (!m_sample_visible)
