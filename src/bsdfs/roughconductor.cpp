 ...
 public:
  RoughConductor(const Properties &props) : BSDF(props) {
    ...
    if (props.has_property("alpha_u") || props.has_property("alpha_v")) {
      if (!props.has_property("alpha_u") || !props.has_property("alpha_v")) Throw(
          "Microfacet model: both 'alpha_u' and 'alpha_v' must be specified.");
      if (props.has_property("alpha")) Throw(
  ...
  std::pair<BSDFSample, Color3> sample(const BSDFContext &ctx,
    ...
    if (!(bs.pdf != 0.f && Frame::cos_theta(bs.wo) > 0.f)) return {bs, 0.f};
    Color3 weight;
    if (m_sample_visible)
    ...
    Color3 F = fresnel_conductor(si.wi.dot(m), m_eta->eval_3(si.geom), m_k->eval_3(si.geom));
    return {bs, (F * weight)};
  }

  Color3 eval(const BSDFContext &ctx, const SceneInteraction &si, const Vector3 &wo) const override {
    Float cos_theta_i = Frame::cos_theta(si.wi),
          cos_theta_o = Frame::cos_theta(wo);
    if (!ctx.is_enabled(BSDFFlags::GlossyReflection) || !(cos_theta_i > 0.f && cos_theta_o > 0.f)) return 0.f;
  ...
  Float pdf(const BSDFContext &ctx, const SceneInteraction &si, const Vector3 &wo) const override {
    Float cos_theta_i = Frame::cos_theta(si.wi),
          cos_theta_o = Frame::cos_theta(wo);
