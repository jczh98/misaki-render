...
#define DEFINE_PROPERTY_ACCESSOR(TYPE, TAG_NAME, SETTER, GETTER)                              \
  void Properties::SETTER(const std::string &name, TYPE const &value, bool warn_duplicates) { \
  ...
  TYPE const &Properties::GETTER(const std::string &name) const {                             \
    ...
    if (!std::holds_alternative<TYPE>(it->second.data))                                       \
      Throw("The property \"{}\" has the wrong TYPE (expected <" #TAG_NAME ">).", name);      \
    return (TYPE const &)it->second.data;                                                     \
  }                                                                                           \
                                                                                              \
  TYPE const &Properties::GETTER(const std::string &name, TYPE const &def_val) const {        \
    const auto it = d->entries.find(name);                                                    \
    if (it == d->entries.end())                                                               \
      return def_val;                                                                         \
    if (!std::holds_alternative<TYPE>(it->second.data))                                       \
...
DEFINE_PROPERTY_ACCESSOR(std::shared_ptr<Component>, component, set_component, component)
DEFINE_PROPERTY_ACCESSOR(const void *, pointer, set_pointer, pointer)

Properties::Properties() : d(new PropertiesPrivate()) {
}

Properties::Properties(const std::string &plugin_name) : d(new PropertiesPrivate()) {
...
struct PropertyTypeVisitor {
  ...
  Type operator()(const std::shared_ptr<Component> &) { return Type::Component; }
...
struct StreamVisitor {
  ...
  void operator()(const std::shared_ptr<Component> &o) { os << o->to_string(); }
...
std::vector<std::pair<std::string, std::shared_ptr<Component>>> Properties::components() const {
  std::vector<std::pair<std::string, std::shared_ptr<Component>>> result;
  result.reserve(d->entries.size());
  for (auto &e : d->entries) {
    auto type = std::visit(PropertyTypeVisitor(), e.second.data);
...
std::shared_ptr<Component> Properties::find_component(const std::string &name) const {
  const auto it = d->entries.find(name);
  if (it == d->entries.end())
    return std::shared_ptr<Component>();

  if (!std::holds_alternative<std::shared_ptr<Component>>(it->second.data))
    Throw("The property \"{}\" has the wrong type.", name);

  return (const std::shared_ptr<Component> &)it->second.data;
}
