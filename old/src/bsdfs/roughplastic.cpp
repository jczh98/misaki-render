 ...
 public:
  RoughPlastic(const Properties &props) : BSDF(props) {
    ...
    if (!distr.is_isotropic())
      Throw(
          "The 'roughplastic' plugin currently does not support "
  ...
  std::pair<BSDFSample, Color3> sample(const BSDFContext &ctx,
    ...
    if ((!has_specular && !has_diffuse) || cos_theta_i <= 0) return {bs, Color3(0)};
  ...
  Color3 eval(const BSDFContext &ctx, const SceneInteraction &si, const Vector3 &wo) const override {
    ...
    if (has_diffuse) {
      ...
      diff /= 1.f - (m_nonlinear ? (diff * fdr)
                                 : Color3(fdr));
      result += diff * (math::InvPi<Float> * (1.f / m_eta / m_eta) * cos_theta_o * t_i * t_o);
    }
    return result;
  }
