...
[[noreturn]] extern MSK_EXPORT void Throw(LogLevel level, const char *file, int line, const std::string &msg);

template <typename... Args>
MSK_INLINE static void Log(LogLevel level, const char *filename, int line, Args &&... args) {
...
#define Log(level, ...)                                    \
  do {                                                     \
    misaki::render::detail::Log(level, __FILE__, __LINE__, \
                                ##__VA_ARGS__);            \
  } while (0)

#define Throw(...)                                           \
  do {                                                       \
    misaki::render::detail::Throw(Error, __FILE__, __LINE__, \
