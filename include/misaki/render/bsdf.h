...
struct MSK_EXPORT BSDFContext {
  ...
  BSDFContext(TransportMode mode, uint32_t type_mask, uint32_t component) : mode(mode), type_mask(type_mask), component(component) {}
  ...
  bool is_enabled(BSDFFlags type_, uint32_t component_ = 0) const {
    ...
    return (type_mask == +BSDFFlags::All || (type_mask & type) == type) && (component == (uint32_t)-1 || component == component_);
  }
};
...
struct BSDFSample {
  ...
  BSDFSample() : wo(Vector3(0)), pdf(0.f), eta(1.f), sampled_type(0), sampled_component(-1) {}
  BSDFSample(const Vector3 &wo) : wo(wo), pdf(0.f), eta(1.f), sampled_type(0), sampled_component(-1) {}
};

class MSK_EXPORT BSDF : public Component {
 public:
