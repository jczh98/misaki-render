...
template <typename Float> struct DiscreteDistribution {
    ...
    void update() {
        ...
        for (uint32_t i = 0; i < size; i++) {
            ...
            if (value < 0.0) {
                throw std::runtime_error(
                    "DiscreteDistribution: entries must be non-negative");
            } else if (value > 0.0) {
                if (m_valid.x() == -1)
                    m_valid.x() = i;
                m_valid.y() = i;
        ...
        if ((m_valid.array() == -1).any()) {
            throw std::runtime_error(
                "DiscreteDistribution: no probability mass found");
        }
        m_sum           = sum;
    ...
    uint32_t sample(Float value) const {
        value *= m_sum;
        return math::binary_search(
            m_valid.x(), m_valid.y() + 1,
            [&](uint32_t index) { return m_cdf[index] < value; });
    }

    Float eval_pmf(uint32_t index) const { return m_pmf[index]; }

    Float eval_pmf_normalized(uint32_t index) const {
        return m_pmf[index] * m_normalization;
    }

    Float eval_cdf(uint32_t index) const { return m_cdf[index]; }
...
template <typename Float> struct Distribution1D {
    ...
    Float pmf(int i) const {
        return (i < 0 || i + 1 >= int(m_cdf.size())) ? 0
                                                     : m_cdf[i + 1] - m_cdf[i];
    }

    uint32_t sample(Float u) const {
        const auto it = std::upper_bound(m_cdf.begin(), m_cdf.end(), u);
        return std::clamp(int(std::distance(m_cdf.begin(), it)) - 1, 0,
                          int(m_cdf.size()) - 2);
    }
...
template <typename Float> struct Distribution2D {
    ...
    std::pair<Eigen::Matrix<Float, 2, 1>, Float>
    sample(const Eigen::Matrix<Float, 2, 1> &u) const {
        const int y = m_marginal.sample(u[1]);
        const int x = m_conditional[y].sample(u[0]);
        Float du1   = (u[0] - m_marginal.cdf()[y]) /
                    (m_marginal.pmf(y) > 0.f ? m_marginal.pmf(y) : 1.f);
        Float du2 =
            (u[1] - m_conditional[y].cdf()[x]) /
            (m_conditional[y].pmf(x) > 0.f ? m_conditional[y].pmf(x) : 1.f);
