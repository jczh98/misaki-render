...
template <typename Float> inline Float schlick_weight(Float cos_theta) {
    return math::power<5>(std::clamp<float>(1 - cos_theta, 0, 1));
}

template <typename Float>
Eigen::Matrix<Float, 3, 1> reflect(const Eigen::Matrix<Float, 3, 1> &wi) {
    return Eigen::Matrix<Float, 3, 1>(-wi.x(), -wi.y(), wi.z());
}

template <typename Float>
Eigen::Matrix<Float, 3, 1> reflect(const Eigen::Matrix<Float, 3, 1> &wi,
                                   const Eigen::Matrix<Float, 3, 1> &n) {
    return Eigen::Matrix<Float, 3, 1>(n) * 2.f * wi.dot(n) - wi;
}

template <typename Float>
Eigen::Matrix<Float, 3, 1> refract(const Eigen::Matrix<Float, 3, 1> &wi,
                                   Float cos_theta_t, Float eta_ti) {
    return Eigen::Matrix<Float, 3, 1>(-eta_ti * wi.x(), -eta_ti * wi.y(),
                                      cos_theta_t);
}

template <typename Float>
Eigen::Matrix<Float, 3, 1> refract(const Eigen::Matrix<Float, 3, 1> &wi,
                                   const Eigen::Matrix<Float, 3, 1> &m,
                                   Float cos_theta_t, Float eta_ti) {
...
std::tuple<Float, Float, Float, Float> fresnel(Float cos_theta_i, Float eta) {
    ...
    Float cos_theta_t_sqr =
        1.f - eta_ti * eta_ti * (1.f - cos_theta_i * cos_theta_i);
...
// Color3 fresnel_conductor(Float cos_theta_i, const Color3 &eta, const Color3
// &k) {
...
// (sometimes referred to as "Fdr").
// https://github.com/mitsuba-renderer/mitsuba2/blob/master/include/mitsuba/render/fresnel.h
template <typename Float> Float fresnel_diffuse_reflectance(Float eta) {
    /* Fast mode: the following code approximates the diffuse Frensel
    ...
    */
    ...
    Float inv_eta = 1.f / eta, inv_eta_2 = inv_eta * inv_eta,
          inv_eta_3 = inv_eta_2 * inv_eta, inv_eta_4 = inv_eta_3 * inv_eta,
          inv_eta_5 = inv_eta_4 * inv_eta;
    if (!eta_l_1) {
        result = 0.919317f - 3.4793f * inv_eta + 6.75335f * inv_eta_2 -
